...
module.exports = {
	name: 'mystats-codwz',
	description: 'search your cod wz stats',
	args: true,
	execute(message, args) {
        const {author } = message
        const {id } = author
        userdb.findOne({discordid: id, Game:'codwz'}, function (err, user) {
            if (err) {
                console.log(err);
                message.channel.send('DB ERR');
            } else {
                let gamertag = user.gertag;
                if (user.platform ==='xbl') {
                ...
                }else if (user.platform ==='psn') {
                    API.login(apiauth.codemail, apiauth.codpass).then(()=>{
                        API.MWwzstats(gamertag, API.platforms.psn).then((data)=>{
                            let username = data.username;
                            let games = data.lifetime.mode.br_all.properties.gamesPlayed;
                            let wins = data.lifetime.mode.br_all.properties.wins;
                            let topten = data.lifetime.mode.br_all.properties.topTen;
                ...
                }else if (user.platform ==='battle') {
                    API.login(apiauth.codemail, apiauth.codpass).then(()=>{
                        API.MWwzstats(gamertag, API.platforms.battle).then((data)=>{
                            let username = data.username;
                            let games = data.lifetime.mode.br_all.properties.gamesPlayed;
                            let wins = data.lifetime.mode.br_all.properties.wins;
                            let topten = data.lifetime.mode.br_all.properties.topTen;
